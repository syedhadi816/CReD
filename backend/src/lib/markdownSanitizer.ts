...
function escapeCurrencyDollarSigns(text: string): string {
  return text.replace(/\$(?=\s*[\d,])/g, (marker, offset) => {
    ...
    const openDollars = countUnescapedToken(before, "$");
    if (openDollars % 2 === 1) {
      // Already inside an inline math span; keep as-is.
      return marker;
    }
    const after = text.slice(offset + 1);
    if (countUnescapedToken(after, "$") > 0) {
      // This dollar has a closer later in the text; treat as math.
      return marker;
    }
    ...
    const afterNum = rest.slice(matchedNumber.length);
    const ch = afterNum[0];
